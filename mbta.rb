...
start_park = find_index(rails, starting_line, intersection)
#find index of park st for ending line
#end_park = rails[ending_line].index('park st')
end_park = find_index(rails, ending_line, intersection)
...
def line_stops (starting_stop, ending_stop)
	stops = (starting_stop - ending_stop).abs
	#stops.abs
end
...
if starting_line == ending_line
	...
	stops = line_stops(start_index, end_index)
	#stops = stops.abs
else
	stops = 0
	#stops to park st on line 1
